...
interface BookingReceivedEmailData extends AppointmentEmailData {
  amountPaid: number;
  totalAmount: number;
}
...
export async function sendBookingReceivedEmail(data: BookingReceivedEmailData) {
  const balance = data.totalAmount - data.amountPaid;
  const info = await transporter.sendMail({
    from: `"Beauty Salon" <${process.env.EMAIL_USER}>`,
    to: data.to,
    subject: "We've Received Your Booking - Beauty Salon",
    html: `
        ...
        <tr>
          <td align="center">
            <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; overflow: hidden;">
              ...
              <tr>
                <td align="center" style="background-color: #a08b7f; padding: 20px 24px;">
              ...
              <tr>
                <td style="padding: 36px 40px 12px 40px;">
                  ...
                  <p style="font-size: 15px; color: #5a4a42; line-height: 1.6; margin: 0 0 28px 0;">
                    Thank you - we've received your booking and payment. Our team will confirm your appointment shortly, and you'll get a follow-up email once it's finalized.
                  </p>
                </td>
              </tr>
              <tr>
              ...
              <tr>
                <td style="padding: 24px 40px 12px 40px;">
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                      <td style="font-size: 14px; color: #5a4a42; padding-bottom: 6px;">Amount paid</td>
                      <td align="right" style="font-size: 14px; color: #2c1810; font-weight: bold; padding-bottom: 6px;">Rs. ${data.amountPaid.toLocaleString()}</td>
                    </tr>
                    ${
                      balance > 0
                        ? `<tr>
                            <td style="font-size: 14px; color: #5a4a42;">Balance due at visit</td>
                            <td align="right" style="font-size: 14px; color: #c47c5a; font-weight: bold;">Rs. ${balance.toLocaleString()}</td>
                          </tr>`
                        : ""
                    }
                  </table>
